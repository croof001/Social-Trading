...
function pub_remove_post(id)
{
	 	$('#st-publish-post-item-'+id).remove();
}
...
$(function() {
	...
	$(".selected-accounts").change(function() {
		var targetHolder = $("#postbox" + $(this).data('account'));
		//The target form holder div
		var targetForm = $("#postbox" + $(this).data('account') + " form");
		//the target form
		...
		if ($(this).is(':checked')) {//does the user select the account
			...
			if (STrootform == targetForm.attr('id')) {
				...
				targetForm.attr('data-isroot', 'true');
				//mark-it as root
				targetForm.attr('data-auto', 'false');
				//mark-it for no auto-fill
				//targetHolder.parent().prepend(targetHolder);  //bring-it to the top
				targetHolder.removeClass('st-faint');
				//root item should be highlight, always
